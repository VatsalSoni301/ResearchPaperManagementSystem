...
def paper_to_be_reviwed():
		...
		if 'id' not in session:
			...
			arrx=notify_expert(arrx)
			count=noticount_expert()
			return render_template("paper_tobe_reviwed.html",name="Paper To Be Reviewed",arr=arr,key=key,arr1=arr1,key1=key1,fname=fname,img=img,c=count,arr2=arrx)
...
def paper_already_review():
		...
		if 'id' not in session:
			...
			arrx=notify_expert(arrx)
			count=noticount_expert()
			return render_template("reviewed_paper.html",name="Reviewed Paper",arr=arr,key=key,fname=fname,img=img,arr1=arrx,c=count)
